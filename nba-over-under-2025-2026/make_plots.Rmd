...
# ---------------------------------------------------------------------------
# Key insight: teams_could_pass + 1 = the worst seed this team could fall to.
# If only 0 opponents can exceed your worst case, you've clinched #1.
# If only 3 can, you've clinched at worst #4 (home court in first round).
#
# Similarly: teams_guaranteed_ahead + 1 = the best seed this team can reach.
# If 6+ opponents are guaranteed more wins, you can't crack the top 6.
# ---------------------------------------------------------------------------
...
clinch_cross <- clinch_data %>%
  select(team_name, conference, max_wins, min_wins) %>%
  inner_join(
    clinch_data %>%
...
clinch_summary <- clinch_cross %>%
  ...
  summarize(
    teams_could_pass = sum(opp_max > min_wins),
    teams_guaranteed_ahead = sum(opp_min > max_wins),
    .groups = "drop"
  ) %>%
  mutate(
    # Worst possible seed = teams_could_pass + 1
    # Best possible seed  = teams_guaranteed_ahead + 1
    ...
    clinch_status = case_when(
      ...
      worst_possible_seed <= 4  ~
        paste0("Clinched #", worst_possible_seed, " or better"),
      worst_possible_seed <= 6  ~ "x - Playoffs",
