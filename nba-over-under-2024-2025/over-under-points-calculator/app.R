...
current_file <- paste0("determined_outcomes_", as.Date(Sys.time()), ".rds")
if (file.exists(current_file)) {
  temp <- read_rds(current_file)
} else {
  temp <- read_rds(paste0("determined_outcomes_", max_date, ".rds"))
}
determined <- temp %>% 
...
display_name <- function(team, outcome_determined = FALSE) {
  if (team == "Blazers" && outcome_determined) "Trail Blazers" else team
}
  ...
  mutate(team_short = str_extract(team, "\\w+$"),  # Extracts the last word now
    ...
    final_table_with_separator <- bind_rows(
      ...
      select(`Rank`, Player, `Points Total`, `Number of<br>Correct Picks`,
             `Correct Picks<br>(Wage 15)`, `Correct Picks<br>(Wage 14)`, `Correct Picks<br>(Wage 13)`)
