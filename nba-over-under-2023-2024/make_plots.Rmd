...
if (nrow(todays_games)) {
  ...
  print_status_change <- function(simulated_standings, game_winner, current_over_under, projections) {
    changes <- simulated_standings %>%
      ...
      mutate(Over_Under_Simulated = if_else(Projected_Wins > win_projection, "OVER", "UNDER")) %>%
      ...
      select(team_name, `Current Over/Under`, Over_Under_Simulated)
    
    if(nrow(changes) > 0) {
      # Mark that changes have been found
