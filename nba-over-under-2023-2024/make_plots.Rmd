  ...
  if(nrow(changes) > 0) {
    ...
    walk(seq_len(nrow(changes)), function(i) {
      ...
      if (row$Can_No_Longer_Win) {
        cat(glue("- If losing today's game, the {row$team_name} clinches UNDER and can no longer reach the OVER projection.\n"))
      } else if (row$Can_No_Longer_Lose) {
        cat(glue("- If winning today's game, the {row$team_name} clinches OVER and can no longer go UNDER based on losses.\n"))
      } else {
