...
$$\mathbb{P}[ (X = 2) \,|\, (X \ge 1)] = \dfrac{\mathbb{P} [ (X = 2) \text{ AND } (X \ge 1) ]}{\mathbb{P}(X \ge 1)}
= \dfrac{\mathbb{P}(X = 2)}{\mathbb{P}(X \ge 1)}$$
$$\mathbb{P}[ (X = 2) \,|\, (X \ge 1)] = \dfrac{\mathbb{P}(X = 2)}{1 - \mathbb{P}(X = 0)} = \frac{e^{-3}\left( \dfrac{3^2}{2!}\right)}{1 - \left[e^{-3}\left( \dfrac{3^0}{0!}\right)\right]} \approx \frac{0.224}{0.950} = 0.236$$
...
<div class="columns-2">
  ...
  - __Question of interest__: are females unfairly discriminated against in terms of promotions?
...
\[
\mathbb{P}(promote\,|\,M) = 21/24 = 0.875 \\
\mathbb{P}(promote\,|\,F) = 14/24 = 0.583
\]
...
![](05B_files/figure-html/simgender-1.png)
...
- Out of the 10,000 simulations, 241 were greater than 0.2916667.

- Therefore, $p$-value = 0.0241.  Since this is smaller than $\alpha = 0.05$, we reject $H_0$ and based on this data, we have evidence that females are unfairly discriminated against in terms of promotions.
