...
picks_wide <- picks %>% 
  ...
  mutate(team = case_when(
    outcome_determined == "OVER" ~ paste(team, "↑"),
    outcome_determined == "UNDER" ~ paste(team, "↓"),
    TRUE ~ team)) %>% 
  separate(
    col = Chester, 
    into = c("Chester", "chester_choice"), 
    sep = " "
  ) %>% 
  separate(
    col = Jake, 
    into = c("Jake", "jake_choice"), 
    sep = " "
  ) %>% 
  separate(
    col = Jenelle, 
    into = c("Jenelle", "jenelle_choice"), 
    sep = " "
  ) %>% 
  separate(
    col = Adonis, 
    into = c("Adonis", "adonis_choice"), 
    sep = " "
  ) %>% 
  separate(
    col = Mike, 
    into = c("Mike", "mike_choice"), 
    sep = " "
  ) %>% 
  separate(
    col = Mary, 
    into = c("Mary", "mary_choice"), 
    sep = " "
  ) %>% 
  separate(
    col = Ryan, 
    into = c("Ryan", "ryan_choice"), 
    sep = " "
  ) %>% 
  separate(
    col = Phil, 
    into = c("Phil", "phil_choice"), 
    sep = " "
  ) %>% 
  ...
  mutate(Chester = if_else(chester_choice == "OVER",
                           paste(Chester, "↑"),
                           paste(Chester, "↓"))) %>% 
  mutate(Jake = if_else(jake_choice == "OVER",
                        paste(Jake, "↑"),
                        paste(Jake, "↓"))) %>% 
  mutate(Jenelle = if_else(jenelle_choice == "OVER",
                           paste(Jenelle, "↑"),
                           paste(Jenelle, "↓"))) %>% 
  mutate(Adonis = if_else(adonis_choice == "OVER",
                          paste(Adonis, "↑"),
                          paste(Adonis, "↓"))) %>% 
  mutate(Mike = if_else(mike_choice == "OVER",
                        paste(Mike, "↑"),
                        paste(Mike, "↓"))) %>% 
  mutate(Mary = if_else(mary_choice == "OVER",
                        paste(Mary, "↑"),
                        paste(Mary, "↓"))) %>% 
  mutate(Ryan = if_else(ryan_choice == "OVER",
                        paste(Ryan, "↑"),
                        paste(Ryan, "↓"))) %>% 
  mutate(Phil = if_else(phil_choice == "OVER",
                        paste(Phil, "↑"),
                        paste(Phil, "↓"))) %>% 
  rename(`Win Target` = win_projection) %>% 
  relocate(`Win Target`, .after = team)
```

```{r message=FALSE, warning=FALSE}
cols_to_delete <- 10:(ncol(picks_wide) - 1)
...
for(i in (cols_to_delete - 9) ) {
  #  do_not_show_list[[i]]$targets <- as.list(cols_to_delete)[[i]]
  do_not_show_list[[i]]$visible <- FALSE
}
...
picks_wide %>% 
  select(Team = team, `Win Target`, 
         Adonis, Chester, Jake, Jenelle, Mary, Mike, Phil, Ryan, 
         everything()) %>% 
  # Doesn't actually sort in DT
  mutate_at(.vars = c("Adonis", "Chester", "Jake", "Jenelle",
                      "Mary", "Mike", "Phil", "Ryan"), 
            .funs = ~ factor(.x, levels = point_levels)) %>% 
#  purrr::map_df(rev) %>% 
  arrange(Team) %>% 
