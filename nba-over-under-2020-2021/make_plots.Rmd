...
player_projections_by_team %>% 
  ...
  filter(Player == current_rankings[1],
         Date == Sys.Date() - 1) %>% 
...
player_projections_by_team %>% 
  ...
  mutate(
    ...
    .before = `Current Projected Points`) %>% 
  select(-`Current Wins`, -`Current Losses`) %>% 
  filter(Player == current_rankings[2],
         Date == Sys.Date() - 1) %>% 
  arrange(desc(`Current Projected Points`)) %>% 
  select(-Player, -Date) %>% 
  DT::datatable(options = list(pageLength = 30, dom = 'ft'))
...
player_projections_by_team %>% 
  ...
  mutate(
    ...
    .before = `Current Projected Points`) %>% 
  select(-`Current Wins`, -`Current Losses`) %>% 
  filter(Player == current_rankings[3],
         Date == Sys.Date() - 1) %>% 
  arrange(desc(`Current Projected Points`)) %>% 
  select(-Player, -Date) %>% 
  DT::datatable(options = list(pageLength = 30, dom = 'ft'))
...
player_projections_by_team %>% 
  ...
  mutate(
    ...
    .before = `Current Projected Points`) %>% 
  select(-`Current Wins`, -`Current Losses`) %>% 
  filter(Player == current_rankings[4],
         Date == Sys.Date() - 1) %>% 
  arrange(desc(`Current Projected Points`)) %>%
  select(-Player, -Date) %>% 
  DT::datatable(options = list(pageLength = 30, dom = 'ft'))
...
player_projections_by_team %>% 
  ...
  mutate(
    ...
    .before = `Current Projected Points`) %>% 
  select(-`Current Wins`, -`Current Losses`) %>% 
  filter(Player == current_rankings[5],
         Date == Sys.Date() - 1) %>% 
  arrange(desc(`Current Projected Points`)) %>% 
  select(-Player, -Date) %>% 
  DT::datatable(options = list(pageLength = 30, dom = 'ft'))
...
player_projections_by_team %>% 
  ...
  mutate(
    ...
    .before = `Current Projected Points`) %>% 
  select(-`Current Wins`, -`Current Losses`) %>% 
  filter(Player == current_rankings[6],
         Date == Sys.Date() - 1) %>% 
  arrange(desc(`Current Projected Points`)) %>% 
  select(-Player, -Date) %>% 
  DT::datatable(options = list(pageLength = 30, dom = 'ft'))
...
player_projections_by_team %>% 
  ...
  mutate(
    ...
    .before = `Current Projected Points`) %>% 
  select(-`Current Wins`, -`Current Losses`) %>% 
  filter(Player == current_rankings[7],
         Date == Sys.Date() - 1) %>% 
  arrange(desc(`Current Projected Points`)) %>% 
  select(-Player, -Date) %>% 
  DT::datatable(options = list(pageLength = 30, dom = 'ft'))
...
player_projections_by_team %>% 
  ...
  mutate(
    ...
    .before = `Current Projected Points`) %>% 
  select(-`Current Wins`, -`Current Losses`) %>% 
  filter(Player == current_rankings[8],
         Date == Sys.Date() - 1) %>% 
  arrange(desc(`Current Projected Points`)) %>% 
  select(-Player, -Date) %>% 
  DT::datatable(options = list(pageLength = 30, dom = 'ft'))
