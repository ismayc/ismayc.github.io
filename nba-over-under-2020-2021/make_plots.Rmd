...
- If either of these outcomes are still possible, "Wins Outcome Determined" will
remain as Not Yet.
...
current_team_wins_to_proj <- team_wins_df %>% 
  group_by(`Team Name`) %>% 
  slice_tail(n = 1) %>% 
  arrange(`Wins Needed`) %>% 
  pull(`Team Name`)
team_wins_track <- team_wins_df %>% 
  ...
  ggplot(aes(x = `Game Number`, y = `Current Win Total`, 
             label = `Current Record`,
             text = `Game Result`,
             color = `Wins \n Outcome \n Determined`)
  ) +
