...
knit: (function(inputFile, encoding) { rmarkdown::render(inputFile, encoding = encoding, output_file = file.path(dirname(inputFile), '2021-nba-over-under.html')) })
---
...
```{r fig.height=10, fig.width=10}
team_track <- ggplot(
  join_with_projections,
  aes(x = `Game Number`, y = `Current Win %`, label = `Current Record`,
      text = `Game Result`)
) +
  geom_point() +
  geom_line() +
...
projected_score_plot <- projected_score %>% 
