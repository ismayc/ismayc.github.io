const P=['Adonis','Andy','Chester','Jake','Mary','Mike','Phil','Ryan'];
...
function renderTab2(el){
  ...
  D.todaysGames.forEach(function(g){
    ...
    if(hU){
      h+='<div><div style="font-size:11px;font-weight:700;color:var(--t2);margin-bottom:6px">'+sn(home)+' ('+hT.w+'-'+hT.l+') &middot; Target: '+hT.wp+' &middot; Wins to go: '+(hT.wtg!==null?hT.wtg:'-')+(hT.wtg===1?' <span style="color:var(--gold)">CLINCH WATCH</span>':'')+'</div>';
      P.forEach(function(p){var pk=D.picks[home][p];h+='<div style="display:flex;align-items:center;gap:6px;padding:2px 0;font-size:12px"><span style="width:55px;color:var(--t2)">'+p+'</span><span class="pill-sm pill-'+pk[1].toLowerCase()+'">'+pk[0]+pk[1][0]+'</span><span style="color:var(--t3);font-size:11px">'+(pk[1]==='OVER'?'wants wins':'wants losses')+'</span></div>'});
      h+='</div>';
    }
    if(aU){
      h+='<div><div style="font-size:11px;font-weight:700;color:var(--t2);margin-bottom:6px">'+sn(away)+' ('+aT.w+'-'+aT.l+') &middot; Target: '+aT.wp+' &middot; Losses to under: '+(aT.ltg!==null?aT.ltg:'-')+(aT.ltg===1?' <span style="color:var(--gold)">CLINCH WATCH</span>':'')+'</div>';
      P.forEach(function(p){var pk=D.picks[away][p];h+='<div style="display:flex;align-items:center;gap:6px;padding:2px 0;font-size:12px"><span style="width:55px;color:var(--t2)">'+p+'</span><span class="pill-sm pill-'+pk[1].toLowerCase()+'">'+pk[0]+pk[1][0]+'</span><span style="color:var(--t3);font-size:11px">'+(pk[1]==='OVER'?'wants wins':'wants losses')+'</span></div>'});
      h+='</div>';
    }
    h+='</div></div>';
  });
...
function renderTab4(el,player){
  ...
  for(var team in D.teams){
    ...
    var od=others.map(function(p){var q=D.picks[team][p];return{player:p,wage:q[0],choice:q[1]}});
  ...
  h+='<div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:14px 20px"><div style="font-size:11px;color:var(--t3)">Biggest Outlier</div><div style="font-size:16px;font-weight:700;color:var(--t1)">'+(rows.length?sn(rows[0].team):'-')+'</div><div style="font-size:11px;color:var(--t3)">'+(rows.length?rows[0].oppositeDir+'/7 others disagree':'')+'</div></div></div>';
  ...
  rows.forEach(function(r){
    ...
    h+='<span style="font-size:12px;color:'+(r.sameDir>=4?'var(--over)':r.sameDir<=2?'var(--under)':'var(--gold)')+'">'+r.sameDir+'/7</span>';
...
var activeTab='rooting',activePlayer='Chester';
function renderAll(){
