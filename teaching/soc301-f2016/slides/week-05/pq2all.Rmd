...
When producing a code for a scatterplot, do we use to `fill` function or the `color` function to change the color of the points? 
...
If you want to plot two variables, where the explanatory is categorical and the response is numeric, which types of graphs could you use?
...
If you want to plot two categorical variables which types of graph could you use?
...
- D. A faceted histogram is for plotting a categorical explanatory variable and a numeric response variable while a faceted barplot is for plotting a categorical response and categorical response variable
...
- C.`bins`, `aes`, and `facet_grid` functions
- D. None of the above. 

# Problem `r lc <- lc + 1; lc`

Which of the following would be the result of the R code in the Console? 

`> library(okcupiddata)`

- A. R will load the `okcupiddata` package.
- B. R will allow you to view the `okcupiddata` dataset.
- C. By performing this output, it will allow you to remove the missing values in the `okcupiddata` dataset.
- D. All of the above.
...
"`Error: could not function 'ggplot'`" appears in R Console after running:
...
- B. `ggplot` is a function of `ggplot2`, so you need to make sure `ggplot2` is loaded via `library(ggplot2)` in a chunk above this chunk
...
Which plot is appropriate to plot the correlation between `sex` and `education` in the `profiles` `okcupiddata` data set?
...
What does the "box" of a box plot represent?

- A. 0, 50th, and 100 percentiles
