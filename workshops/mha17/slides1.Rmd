...
### A frequency distribution shows the values that a variable can take and the number of obser-vations associated with each value. 
...
- Use _mode_ when the data are qualitative, categorical, or nominal (e.g., eye color, political party) and values can only fit into one category (i.e., mutually-exclusive)
- Use _median_ when you have extreme (numerical) scores
- Use _mean_ when the data does not include extreme scores (i.e., outliers) and are not categorical
...
- $range = h – l$
    ...
    where $h$ - highest numerical value and <br> $l$ is lowest numerical value 
